...
def test_next_raid_reminder_time_et_skips_non_raid_days():
    now = datetime(2026, 9, 25, 21, 0, 0, tzinfo=ET)
    assert _next_raid_reminder_time_et(now) == datetime(2026, 9, 28, 20, 45, 0, tzinfo=ET)
...
def test_absence_setup_prompt_text_uses_add_absence_cta():
    from bot.cogs.absence_cog import _absence_setup_prompt_text
    assert 'Add Absence' in _absence_setup_prompt_text()
